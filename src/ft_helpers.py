...
def load_data(full = True, preprocessed=False):
    ...
    if preprocessed:
        ...
        test['body'] = load_from_txt(PREPROCESSED_DATA_TEST)
...
def create_probabilities_csv(model, test, filename):
    ...
    # Convert labels back to -1's and 1's
    test['label'] = test['label'].str.replace('__label__sadface','-1')
    test['label'] = test['label'].str.replace('__label__happyface','1')
    test = test[['id', 'label', 'prob']]
